...
{
	/// <summary>
	/// The Json returned by CakeMail when counting the number of records looks like this:
	///	{
	///		count: 1234
	///	}
	/// The goal of this JsonConverter is to ignore the "count" property and to return the numerical value.
	/// </summary>
	public class CakeMailCountOfRecordsConverter : JsonConverter
	{
		...
		public override object ReadJson(JsonReader reader, Type objectType, object existingValue, JsonSerializer serializer)
		{
			if (reader.TokenType == JsonToken.StartObject)
			{
				...
				// Read the next token, presumably the value
				reader.Read();

				// Parse the numerical value
				var valueSerializer = new JsonSerializer();
				return valueSerializer.Deserialize(reader, objectType);
			}
